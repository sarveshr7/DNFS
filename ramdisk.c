...
static int ramdisk_do_write(const char *path, const char *buf, size_t size, off_t offset)
{
	node* curr_file;
	...
	if (ramdisk_expand(path, offset + size))
		return -ENOMEM;

	memcpy(curr_file->data + offset, buf, size);
...
static int ramdisk_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	return ramdisk_do_write(path, buf, size, offset);	
}
...
static int ramdisk_read(const char *path, char *buf, size_t size, off_t offset,struct fuse_file_info *fi)
{
	...
	if (offset >= curr_file->file_size)
		return 0;

	if (size > curr_file->file_size - offset)
		size = curr_file->file_size - offset;
